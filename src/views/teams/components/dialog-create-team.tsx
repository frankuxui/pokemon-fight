...
export default function DialogCreateTeams ({ open, setOpen }: { open: boolean; setOpen: (_open: boolean) => void; }) {
  ...
  const onSubmit: SubmitHandler<FormProps> = (data: FormProps) => {
    startTransition(() => {
      createTeam({ ...data, avatar: selectedAvatar ? `../avatars/${selectedAvatar}.png` : '' })
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <Dialog.Content className="max-w-xl overflow-hidden max-h-[calc(100vh_-_4rem)] flex flex-col">
        <Dialog.Header className='flex items-center justify-between p-6'>
          <div className="grid">
            <h2 className="text-lg font-semibold">Crear Equipo</h2>
            <p className="text-sm">Formulario para crear un nuevo equipo</p>
          </div>
                      ...
                      <div className="grid grid-cols-6 gap-4">
                        {
                          teamAvatars.map((team) => (
                            <motion.button
                              ...
                              key={team.id}
                              type="button"
                              onClick={() => setSelectedAvatar(team.name)}
